...
public class RestaurantController {
    ...
    // ✅ SIMPLIFIED & RELIABLE: Get orders for a specific restaurant by ID (direct query)
    @GetMapping("/orders")
    public List<Order> getRestaurantOrders(@RequestParam Long restaurantId) {
        return orderRepository.findByRestaurantId(restaurantId);
    }

    // Additional endpoint to get orders by restaurant (if you need the old owner-based one, keep separate)
    // But we already have /orders/by-restaurant (optional, kept for compatibility)
    @GetMapping("/orders/by-restaurant")
    public List<Order> getOrdersByRestaurantId(@RequestParam Long restaurantId) {
        return orderRepository.findByRestaurantId(restaurantId);
    }
}