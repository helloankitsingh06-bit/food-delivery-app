...
public class RestaurantService {
    ...
    // ✅ Get restaurants by owner email – returns List (uses findAllByOwner)
    public List<Restaurant> getRestaurantsByOwner(String ownerEmail) {
        User owner = userRepository.findByEmail(ownerEmail)
            .orElseThrow(() -> new RuntimeException("Owner not found with email: " + ownerEmail));
        return restaurantRepository.findAllByOwner(owner);   // ← FIXED: use findAllByOwner
    }
    ...
    // Get restaurant by ID (used in OrderService)
